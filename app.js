...
const DEFAULT_LEVEL_RULES = [
  { code: "gran_mago", name: "Gran Mago", min_xp: 600 },
  { code: "maestro", name: "Maestro", min_xp: 450 },
  { code: "prefecto", name: "Prefecto", min_xp: 300 },
  { code: "destacado", name: "Estudiante destacado", min_xp: 200 },
  { code: "alumno", name: "Alumno", min_xp: 100 },
  { code: "aprendiz", name: "Aprendiz", min_xp: 0 },
];

const DEFAULT_BADGE_RULES = [
  { icon: "✨", name: "Primer Hechizo", rule_type: "completed_courses", threshold_int: 1 },
  { icon: "⚡", name: "Ruta en Marcha", rule_type: "completed_courses", threshold_int: 3 },
  { icon: "⭐", name: "Prefecto/a", rule_type: "xp", threshold_int: 300 },
  { icon: "♛", name: "Gran Mago de las Habilidades Blandas", rule_type: "all_available" },
  { icon: "✉", name: "Mensajero de las Casas", rule_type: "course_completed", course_code: "comunicacion" },
  { icon: "♧", name: "Guardian de la Resiliencia", rule_type: "course_completed", course_code: "resiliencia" },
  { icon: "⌚", name: "Custodio del Tiempo", rule_type: "course_completed", course_code: "organizacion" },
  { icon: "♫", name: "Cronista Magico", rule_type: "course_completed", course_code: "storytelling" },
];

function level(xp) {
  const rules = (levelRules.length ? levelRules : DEFAULT_LEVEL_RULES)
    .slice()
    .sort((a, b) => Number(b.min_xp) - Number(a.min_xp));
  return rules.find(rule => xp >= Number(rule.min_xp))?.name || "Aprendiz";
}
  ...
  try {
    ...
    if (error) {
      ...
      if (message.includes("email not confirmed")) {
        toast("Tu cuenta todavia no esta activa. Solicita una contrasena temporal al administrador o focal.", "warning", 12000);
      } else {
...
async function loadDashboardData() {
  await loadHouses();
  const [courseRes, progressRes, levelRes, badgeRes] = await Promise.all([
    ...
    sb.rpc("academy_get_level_rules"),
    sb.rpc("academy_get_badge_rules")
  ]);
  if (courseRes.error) throw courseRes.error;
  if (progressRes.error) throw progressRes.error;
  courses = courseRes.data || [];
  progress = progressRes.data || [];
  levelRules = levelRes.error ? DEFAULT_LEVEL_RULES : levelRes.data || DEFAULT_LEVEL_RULES;
  badgeRules = badgeRes.error ? DEFAULT_BADGE_RULES : badgeRes.data || DEFAULT_BADGE_RULES;
}

function completedCount() {
  return progressForAvailableCourses().filter(item => item.completed).length;
}

function myPoints() {
  if (me.role !== "student") return 0;
  return progressForAvailableCourses().filter(item => item.completed).reduce((sum, item) => sum + (item.course?.points || CONFIG.pointsPerCourse), 0);
}

function myXp() {
  return progressForAvailableCourses().filter(item => item.completed).reduce((sum, item) => sum + (item.course?.xp || CONFIG.xpPerCourse), 0);
}
...
function isCourseAvailable(course) {
  if (course.release_status === "coming_soon") return false;
  if (!course.available_at) return true;
  return new Date(course.available_at).getTime() <= Date.now();
}

function deployedCourses() {
  return courses.filter(isCourseAvailable);
}

function progressForAvailableCourses() {
  const availableIds = new Set(deployedCourses().map(course => course.id));
  return progress.filter(item => availableIds.has(item.course_id));
}

function formatCountdown(target) {
  if (!target) return "";
  const diff = new Date(target).getTime() - Date.now();
  if (diff <= 0) return "Se habilitara pronto.";
  const days = Math.floor(diff / 86400000);
  const hours = Math.floor((diff % 86400000) / 3600000);
  const minutes = Math.floor((diff % 3600000) / 60000);
  if (days > 0) return `Disponible en ${days} dia(s) y ${hours} h.`;
  if (hours > 0) return `Disponible en ${hours} h y ${minutes} min.`;
  return `Disponible en ${Math.max(1, minutes)} min.`;
}
...
function renderProfile() {
  ...
  const totalAvailable = deployedCourses().length;
  $("progressText").textContent = `${completedCount()}/${totalAvailable}`;
  $("progressFill").style.width = `${totalAvailable ? Math.round(completedCount() / totalAvailable * 100) : 0}%`;
...
function renderCourses() {
  ...
  courses.forEach((course, index) => {
    ...
    const head = element("div", { className: "course-head" }, [
      ...
      element("span", {
        className: `state ${available && done ? "done" : "pending"}`,
        text: !available ? "Proximamente" : done ? "Aprobado" : "Pendiente"
      })
    ]);
    ...
    const requirements = [
      ...
      requiresAssignment ? "Requiere tarea" : "Sin tarea",
    ].join(" · ");
    card.append(head, element("h3", { text: course.title }), element("p", { text: course.description || "" }));
    if (available) {
      if (hasExam) card.append(element("p", { text: stats }));
      card.append(actions);
    } else {
      const message = course.coming_soon_text || "Este curso se habilitara proximamente para que avances por etapas.";
      const countdown = formatCountdown(course.available_at);
      card.append(element("div", { className: "empty-state", text: countdown ? `${message} ${countdown}` : message }));
    }
...
function renderBadges() {
  ...
  const items = [
    ...
    ["★", "Prefecto/a", xp >= 300],
    ["♛", "Gran Mago de las Habilidades Blandas", totalAvailable > 0 && completedCount() === totalAvailable],
...
function isBadgeUnlocked(rule) {
  const xp = myXp();
  const totalAvailable = deployedCourses().length;
  if (rule.rule_type === "completed_courses") return completedCount() >= Number(rule.threshold_int || 0);
  if (rule.rule_type === "xp") return xp >= Number(rule.threshold_int || 0);
  if (rule.rule_type === "all_available") return totalAvailable > 0 && completedCount() === totalAvailable;
  if (rule.rule_type === "course_completed") return isCourseCompleted(rule.course_code);
  return false;
}

function renderBadges() {
  const items = (badgeRules.length ? badgeRules : DEFAULT_BADGE_RULES).map(rule => [
    rule.icon || "*",
    rule.name,
    isBadgeUnlocked(rule)
  ]);
  const grid = clear($("badgesGrid"));
  items.forEach(item => grid.append(element("article", { className: `badge ${item[2] ? "unlocked" : ""}` }, [
    element("div", { className: "badge-icon", text: item[0] }),
...
async function renderAdminIfAllowed() {
  if (["focal", "admin"].includes(me.role)) await renderAdmin();
}
...
async function init() {
  ...
  $("closeOnedriveGuideBtn").onclick = () => $("onedriveGuideDialog").close();
  await initSession();
}
